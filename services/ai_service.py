...
class AIService:
    """Service for AI content generation with singleton pattern"""
    ...
    def _initialize_ai(self):
        ...
        try:
            ...
            self._llm_instance = llm_factory.select_best_model(
                min_intelligence=8,
                min_speed=6,
    ...
    def generate_content(self, form_data: FormData) -> Tuple[str, str]:
        """
        Generate AI content for investment recommendation
        ...
        """
        if not self._ai_available or self._llm_instance is None:
            raise RuntimeError("AI service is not available. Please ensure ai_tools and prompts modules are installed.")
        
        try:
            # Generate investment summary (returns markdown)
            executive_summary_markdown = self._generate_investment_thesis(form_data)
            
            # Generate investment rationale (returns markdown)
            investment_rationale_markdown = self._generate_analysis(form_data)
    ...
    def _generate_investment_thesis(self, form_data: FormData) -> str:
    ...
    def _generate_analysis(self, form_data: FormData) -> str:
    ...
    def _generate_fallback_content(self, form_data: FormData) -> Tuple[str, str]:
        """Generate fallback content when AI is not available"""
        thesis_markdown = self._generate_fallback_thesis(form_data)
        analysis_markdown = self._generate_fallback_analysis(form_data)
        
        # Store original Markdown content for PDF generation
        form_data.ai_executive_summary_markdown = thesis_markdown
        form_data.ai_investment_rationale_markdown = analysis_markdown
        
        # Convert Markdown to HTML for QuillJS display
        thesis_html = self._format_text_for_display(thesis_markdown)
        analysis_html = self._format_text_for_display(analysis_markdown)
        
        # Store HTML content for QuillJS
        form_data.ai_executive_summary = thesis_html
        form_data.ai_investment_rationale = analysis_html
        
        return thesis_html, analysis_html
